      ...
      if (body.length > 0) {
        try {
          ...
          if (rawModel && codexCreds && decision.provider === 'codex') {
            ...
            // The chain entry must be one the Claude pool can POSITIVELY serve:
            // "not a Codex slug" also describes `gpt-4o`, a typo, and — when
            // discovery is down and the slug set is empty — every entry in the
            // chain, none of which Anthropic can answer.
