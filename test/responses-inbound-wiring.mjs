...
const codexStub = createServer((req, res) => {
  ...
  req.on('end', async () => {
    ...
    // wide so a loaded runner (the suite runs eight files at once) cannot
    // push a correct run past the bound; 300/250 flaked at 16.
    await sleep(1500);
    res.end();
  });
});
...
header('D. errors, non-stream, and the stateless rule');
{
  ...
  const r3 = await fetch(`${BASE}/v1/responses`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ model: CLAUDE_MODEL, input: 'hi', stream: true, previous_response_id: 'resp_1' }) });
  const e3 = await r3.json();
  check('previous_response_id on the Claude pool → 400 after routing, OpenAI envelope, param named, no upstream request', r3.status === 400 && e3.error.param === 'previous_response_id' && /ChatGPT-subscription/.test(e3.error.message), JSON.stringify(e3));
  const c3 = codexSeen.bodies.length;
  const r3b = await post({ model: CODEX_SLUG, input: 'continue', stream: true, store: true, previous_response_id: 'resp_prev' });
  const sent3 = codexSeen.bodies.at(-1);
  check('previous_response_id on a ChatGPT-subscription model → forwarded untouched to the backend (store kept)', r3b.res.status === 200 && codexSeen.bodies.length === c3 + 1 && sent3.previous_response_id === 'resp_prev' && sent3.store === true, JSON.stringify(sent3));
  const r3c = await post({ model: CODEX_SLUG, input: 'hi', stream: true });
  const sent3c = codexSeen.bodies.at(-1);
  check('store omitted by the client stays omitted (the backend applies its default)', r3c.res.status === 200 && !('store' in sent3c) && sent3c.stream === true, JSON.stringify(sent3c));
