...
| `--model=<name>` | Force a model. Shortcuts (`fable`, `opus`, `sonnet`, `haiku`, `fable1m`), full IDs (`claude-fable-5`, `claude-opus-5`), or a **provider prefix** (`openai:gpt-4o`, `groq:llama-3.3-70b`, `claude:fable`, `claude:opus`, `local:qwen-coder`) to force the backend server-wide. | passthrough |
