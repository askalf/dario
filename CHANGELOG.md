...
- **Wire-tighten the billing block + per-model beta to real CC 2.1.199 (#657)** — live captures of the genuine `claude` 2.1.199 binary (loopback MITM, sdk-cli entrypoint — the entrypoint dario claims) showed three request signals had drifted from what dario emits. None was breaking subscription routing (the classifier isn't hard-validating them today), but each was a clean dario-vs-CC tell. All now match the live binary:
  - **`cch` billing token dropped.** CC stopped stamping the `cch` integrity token between 2.1.177 and 2.1.199 — the block is now `cc_version=…; cc_entrypoint=sdk-cli;` with no `cch=`. dario was appending a *random* one (`CCH_SEEDS` covers only 2.1.177), a token genuine CC no longer sends. Emission is now gated on `hasCchSeed()`: the deterministic value when a calibrated seed exists, otherwise the token is omitted entirely — matching current CC. Fail-safe flips from "random" to "omit".
  - **Per-model `anthropic-beta` matrix refreshed.** `betaForModel` encoded CC 2.1.170's rules. Updated to 2.1.199: sonnet-5 now keeps `mid-conversation-system` (dario had dropped it, a sonnet-4-6 rule), haiku drops `afk-mode` and emits `claude-code-20250219` in position 5, fable places `fallback-credit` immediately before `afk-mode`, and `[1m]` carries `context-1m` at position 2. Rebuilt with anchor-relative insert/move so the order matches, not just membership; correct whether or not the base carries the remote-config-volatile `afk-mode`.
  - **`cc_version` build suffix is now stable per config.** A clean-room capture (fresh HOME, no CLAUDE.md/memory) proved the suffix tracks the *system context*, not the user message — every prompt returned the same value. dario's `chars[4,7,20]`-of-user-message hash was wrong-input and, worse, per-request-varying where real CC emits one stable suffix per install. Replaced with a stable, memoized value derived from the replayed template.
  - **Drift + billing guards.** New `scripts/check-wire-drift.mjs` (self-hosted `wire-drift-self-hosted.yml`) runs the real binary and fails if `betaForModel` or the cch gate diverge from live CC — the runtime counterpart to the static `cc-drift-watch`, which never saw any of this. `scripts/check-overage-live.mjs` (`npm run check:overage`) is an operator-run harness that confirms all models stay on the subscription pool under load. Verified: 105 tests green, wire-drift clean vs live CC 2.1.199, and live billing on `five_hour` across every model incl. the `[1m]` variants.
## [4.8.128] - 2026-07-03
