...
- **Template labels follow Claude Code 2.1.281.** `_version`, `_supportedMaxTested` and the
  `user-agent` header now read `2.1.281`. The request shape is unchanged, so `_captured` keeps
  the date of the last real capture.
