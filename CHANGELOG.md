...
- **A request that finishes during the SIGTERM drain is counted.** Shutdown closed the ledger and ended the request log before it waited for in-flight requests, so the long streamed runs the drain exists for (dario#1370) finished with their spend missing from the lifetime ledger, from their key's daily budget total and from the log. The ledger flushes while the drain waits and closes after it, as do the key store and the request log.
- **A budgeted key's in-flight reservation is priced at the model the request is billed as.** It was priced on the client's `model` as written, so a short name (`fable`, `opus`) or a server-wide `--model` override fell back to the sonnet-4-6 rate: a third of fable's, a quarter of opus-5-5's under `--model=opus` on a haiku request. A concurrent burst on the key could then complete for several times its cap (dario#1378). The reservation now resolves the model the way the request path does (operator alias, provider prefix, effort suffix, family shorthand) and takes the costliest of it and any `--model` / `--fast-model` override. A model with no published rate is reserved at the highest listed one rather than a cheaper fallback. Model lookups on the alias tables read own keys only, so a client model named `__proto__` is an unknown name, not Object.prototype.
- **An operator-set delay past 2^31 - 1 ms no longer fires every millisecond.** Node replaces a longer timer delay with 1 ms, so `DARIO_CODEX_USAGE_POLL_MS=3000000000` turned the idle-seat poll into a 1 ms loop of account reloads and `/wham/usage` calls (dario#1400), and a `DARIO_SHUTDOWN_GRACE_MS` that large fired the force-exit at once and skipped the drain (dario#1370). Every delay an operator sets that reaches a timer is now capped at 2^31 - 1 ms (about 24.8 days): the codex usage poll, the shutdown grace (less the force-exit guard's 5 s margin), `--queue-timeout`, `--pool-shared-state-interval`, the overage guard's cooldown, pacing and think-time delays, the upstream timeout, `DARIO_PROBE_TIMEOUT_MS` and `dario doctor --auth-check --timeout-ms`.
...
- **Template labels follow Claude Code 2.1.282.** `_version`, `_supportedMaxTested` and the `user-agent` header now read `2.1.282`. A live capture against Claude Code 2.1.282 matched the bundled template, so the request shape is unchanged and `_captured` keeps the date of the last real capture.
## [6.12.1] - 2026-09-24
...
- **A rejected Codex token is refreshed and retried, then cooled** (dario#1342, dario#1343) — the Codex backend answering 401/403 on a token dario's clock still trusts now triggers one forced refresh and one retry; if it still fails, the seat is reported unavailable, so the chain fails over and selection stops handing it the next request instead of relaying a 401 to the client. `getFreshCodexAccount` only ever refreshed on the clock, so a token revoked upstream was re-sent unchanged on every request — six hours of `Upstream Codex backend error` on a fleet whose stored token was valid for another day. `forceRefreshCodexAccount` (same single-flight and failure cool-down as the clock path), `isCodexAuthFailure` and `refreshAfterCodexAuthFailure` are exported.
...
- **CC drift patch** — `SUPPORTED_CC_RANGE.maxTested` bumped `2.1.269` → `2.1.270` for CC v2.1.270. Auto-drafted by `cc-drift-watch.yml`. Template re-capture, if needed, is auto-handled by `cc-drift-template-watch.yml`.
## [6.6.4] - 2026-09-12
...
- **README line-count claim refreshed `~32k` → `~33k`.** `src/` is 33,005 lines across 68 files; this PR's 62 carried the total past `check-readme-line-count.mjs`'s ±1000 tolerance. Both claims updated — the hero line the guard reads, and the Source row in the comparison table, which the guard does not read and which would otherwise have drifted on unnoticed.
...
- **Template label refresh** — `_version`, `_supportedMaxTested`, and the `user-agent` header bumped to `2.1.266` to track `@anthropic-ai/claude-code@latest`. The live wire shape is unchanged — cc-drift-template-watch ran `capture-and-bake --check` against live CC v2.1.266 and found zero shape drift vs the bundle — so this is a label refresh, not a re-capture (`_captured` stays at the last real capture). Auto-merged; clears the `sdk-drift` early-warning signal.
## [6.0.42] - 2026-09-09

- **CC drift patch** — `SUPPORTED_CC_RANGE.maxTested` bumped `2.1.265` → `2.1.266` for CC v2.1.266. Auto-drafted by `cc-drift-watch.yml`. Template re-capture, if needed, is auto-handled by `cc-drift-template-watch.yml`.
## [6.0.41] - 2026-09-08
...
- **The non-window cool-down cannot be bypassed by the fallback paths.** Caught in review on #1264: `isParkedInLiveWindow` reports false for a cooling seat by design, and both fallback filters (the all-exhausted branch of `select()` and the mid-flight `selectExcluding()`) tested only that — so a seat that had just answered `retry-after: 17` could be retried inside the same client request, which is the retry storm the cool-down exists to prevent. Both now filter on one named predicate, `isProbeable`, so a future not-askable state cannot be added to one path and missed by the other. `parkedUntil` / `parkedCount` count a cooling seat too, so a pool that is entirely parked-or-cooling answers an honest local 429 with `retry-after` at the earliest return instead of spending a probe that can only 429 again. An auth cool-down still does not count as a rate limit.
...
- **Shared pool state across instances: `--pool-shared-state` (#993 follow-up, the two "not solved" items in multi-instance.md).** Two replicas behind one Service each kept their own rate-limit accounting and sticky bindings, so each believed a seat had full headroom, each ate its own 429 to learn otherwise, and a conversation re-cached its prompt prefix on a different seat per replica. With `DARIO_POOL_SHARED_STATE=1` (or `--pool-shared-state`) every instance reports each reading it takes to the refresh-lock service it already shares and pulls the others' readings every two seconds (`--pool-shared-state-interval=MS`), adopting any newer than its own: a 429 taken on one instance parks the seat on all of them within a pull, logged on each as `seat "busy" parked by peer <id>'s reading: …`, with `rejectedCount` staying each instance's own and `readingFrom` on `GET /accounts` / `reading_from` on `GET /admin/accounts` saying whose reading a seat carries. Sticky bindings are published on bind and on failover rebinding, and an instance with no binding of its own asks the service before choosing, so the second turn of a conversation that started on the other replica reads the cache the first turn wrote. Both reference backends (`redis-lock/`, `cloudflare/refresh-lock/`) serve the three new endpoints under the same bearer; what crosses the wire is snapshots, aliases and hashed sticky keys. Fails open like the lock: an unreachable service is one log line per outage, and `GET /accounts` carries a `sharedState` block with the instance id, last pull, adoption and error counts. Off by default; off is byte-identical to before.
- **Requests can name their consumer; dario attributes by it and can pace by it (#1244 follow-up).** A pool shared by a team served everyone through one `DARIO_API_KEY` with no way to say whose traffic went where, and one heavy user could hold every concurrency slot. An `x-dario-consumer` header (one printable token, ≤64 chars) names the consumer; without it, attribution falls back to a hash of the body's user id — the Anthropic `metadata.user_id` with Claude Code's per-session suffix dropped so one person is one key across sessions, or the OpenAI `user` field — so no account id or raw user id becomes an analytics key. `GET /analytics` gains `perConsumer` (requests, tokens, cache share, estimated cost, the seats the consumer landed on, last model); request log lines and the `-v` usage line carry `consumer`; the TUI's Hits tab shows it. `--max-concurrent-per-consumer=N` (`DARIO_MAX_CONCURRENT_PER_CONSUMER`, default 0 = off) caps in-flight requests per named consumer: a consumer at the cap waits in the queue while slots stay free for everyone else, and a freed slot goes to the first waiter whose consumer is under its cap, so one user's backlog never holds up another's next turn. The queue snapshot on `/analytics` and `/health` reports `maxConcurrentPerConsumer` and `consumersActive`. Only the header paces (the body is parsed after the slot is taken); unnamed requests are never capped.
...
- **The npm package listing describes both plans (#1246).** `description` and `keywords` now match the README's two-plan story — Claude and ChatGPT, either wire shape, automatic failover; keywords add chatgpt, codex, claude-code, cursor, cline, aider, agent-sdk and llm-proxy and drop the generic ai / api.
