...
Two things enforce it. `scripts/check-issue-close-keywords.mjs` fails CI on a PR whose title or body would auto-close somebody else's issue, and `.github/workflows/reopen-user-issues.yml` reopens one that got closed anyway. Your own issues and the drift watchers' bot-filed alerts are exempt — `Fixes #N` those freely.
